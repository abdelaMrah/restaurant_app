...
class ApiService {
  ...
  constructor() {
    ...
    this.api?.interceptors.response.use(
      (response) => {
        this.emitHeaders(
          response.headers["x-permissions"]
            ? (response.headers["x-permissions"] as string).split(", ")
            : undefined
        );
  ...
  public post<T>(
    ...
    params: Record<string, any> = {}
     ...
     return this.api.post<T>(endpoint, data, params);
  }
  ...
  public patch<T>(
    ...
    return this.api.patch<T>(endpoint, data, params);
  }
